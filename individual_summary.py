...
for gene_file, guide_file in zip(gene_individual_files, individual_files):
    process_data(gene_file['path'], guide_file['path'], ref_dict, gene_file['title'])



# var_combined = pd.concat([adata.var for adata in h5ad_list], axis=0)
# #X_combined = np.concatenate([adata.raw.X for adata in h5ad_list], axis=0)
# adata_merged = AnnCollection(h5ad_list).to_adata
# adata_merged.var = var_combined

# # Path for the merged file
# merged_h5ad_path = 'merged.h5ad'
